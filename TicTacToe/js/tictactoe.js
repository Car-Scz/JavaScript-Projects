...
watch()};
function watch() {
	// disable the stop button since the game has not started
...
function rollForTurn() {
	...
	// play dice sounds during the game roll for turn
	diceRoll();
	...
	for (i=0;i<xArray.length;i++) {
		...
		var pTwo = xArray[1];
		if (pOne == pTwo) {
			// rigging roll on tie to avoid bug in code. Need to address this later...
			pOne = 1;
			pTwo = 2;
		}
		...
		// time delay for dramatic affect
...
function startGame() {
	...
	if (activePlayer == "") {
		// if it was a tie, then reroll
		activePlayer = rollForTurn();
	}
	setTimeout(function() {hideGameMsg();}, 4000);
	// assign proper state of the control buttons
	// disable the start button since the game is now afoot
	var btn = document.getElementById("btnStart");
	btnDisabled(btn);
	// enable the stop button since the game is now afoot
	var btn = document.getElementById("btnStop");
	stopEnabled(btn);
...
function determineAvatar() {
	...
	// returns an array of both player"s assigned avatars
...
function avatarPlaced() {
	...
	check4Tie();
	// call this function to inquire if there was a cat"s game.
}
...
function recordMoves(square) {
	...
	// separate the string by commas to create an array
	verdict = check(info,square);
	// call function to check if proposed square is already occupied
	return verdict;
}
...
function winner(winDetected,winCon) {
	if (winDetected == "win") {
		...
		// call function to make the gameboard pulse with colors
		glowBoard(winCon);
	}
}
...
function squareSound() {
	...
	sound.play();
	// add delay to these to keep sound short
...
// -------------------------------------------------------------
// checking for wincon squares 0,1,2
function checkWinCon1(info,squareArray) {
	...
	for (var i in info) {
		if (info[i].charAt(0) == "0") {
			...
			// only interested in recording the avatar
		}
	...
	if (match0Avatar != undefined && match1Avatar != undefined && match2Avatar != undefined) {
		if (match0Avatar == match1Avatar && match0Avatar == match2Avatar) {
			winDetected = "win";
			// this flag will pass when a win has been detected
			winner(winDetected,winCon1);
			return;
		}
	}
	winner(winDetected,winCon1);
	// winCon1 is the array of win combo
}
...
// checking for wincon squares 0,3,6
...
// checking for wincon squares 1,4,7
...
// checking for wincon squares 2,5,8
...
// checking for wincon squares 0,4,8
